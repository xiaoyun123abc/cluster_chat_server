...
int main(int argc, char **argv)
{
    ...
    //连接服务器成功，启动接收子线程
    std::thread readTask(readTaskHandler, clientfd);        //pthread create
    ...
    for (;;)
    {
        ...
        int choice = 0;
        cin >> choice;
        cin.get();              // 读掉缓冲区残留的回车,输入一个整数->实际上是输入一个整数+回车，如果不把回车读取，那么后续读字符串时，会误将回车读取
        ...
        switch (choice)
        {
            case 1: // login业务
            {
                ...
                sem_wait(&rwsem);       //阻塞在此，等待信号量，由子线程处理完登录的响应消息后，通知这里
                if (g_isLoginSuccess)
                {
                    //进入聊天主菜单页面
            ...
            case 2: // Register业务
            {
                ...
                sem_wait(&rwsem);       //阻塞在此，等待信号量,子线程处理完注册消息会通知
            }
            break;
            case 3:   //quit业务
            {
...
void doLoginResponse(json &responsejs)
{
    ...
    else // 登录成功
    {
        ...
        if (responsejs.contains("friends"))
        {
            ...
            vector<string> vec = responsejs["friends"];   //对其中每一个string进行解码
            for (string &str : vec)
            {
...
void readTaskHandler(int clientfd)
{
    for (;;)
    {
        ...
        //将接收自ChatServer转发的数据，反序列化生成json数据对象
